...
import com.xinwo.cache.DiskCache;
...
public class DiskLruCache implements DiskCache<String, String> {
    ...
    public static DiskLruCache open(long maxSize, int appVersion) {
        File directory = XinApplicationUtil.Companion.getInstance().getMApplication().getCacheDir();
        if (maxSize <= 0) {
