...
#include <dlfcn.h>
#include <fcntl.h>
#include <xin_linker/xin_dlfcn.h>
...
XIN_EXPORT void *DlFcn::dlopen(const char *lib_name, int flags) {
...
XIN_EXPORT void *DlFcn::dlsym(void *handle, const char *name) {
  ALOGD("dlsym %s", name);
  KCHECKP(handle)
  if (android_api_ != __ANDROID_API_N__) {
...
XIN_EXPORT int DlFcn::dlclose(void *handle) {
  if (android_api_ != __ANDROID_API_N__) {
...
XIN_EXPORT void *DlFcn::dlopen_elf(const char *lib_name, int flags) {
  dl_iterate_data data{};
  data.info_.dlpi_name = lib_name;
  if (!dl_iterate_phdr) {
    return nullptr;
...
XIN_EXPORT void *DlFcn::dlsym_elf(void *handle, const char *name) {
  KCHECKP(handle)
  int k;
...
XIN_EXPORT int DlFcn::dlclose_elf(void *handle) {
  KCHECKI(handle)
  auto *ctx = (struct ctx *)handle;
  if (ctx->dynsym)
    free(ctx->dynsym);
