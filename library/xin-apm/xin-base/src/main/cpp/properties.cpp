...
std::string GetProperty(const std::string &key, const std::string &default_value) {
  ...
  xin__system_property_read_callback(
      pi,
...
static const prop_info *WaitForPropertyCreation(const std::string &key,
  ...
  while ((pi = __system_property_find(key.c_str())) == nullptr) {
    ...
    if (!xin__system_property_wait(nullptr, global_serial, &global_serial, &ts))
      return nullptr;
  }
  return pi;
}
...
bool WaitForProperty(const std::string &key, const std::string &expected_value,
  ...
  while (true) {
    ...
    xin__system_property_read_callback(pi, WaitForPropertyCallback, &data);
    if (data.done)
      return true;
    ...
    UpdateTimeSpec(ts, relative_timeout, start_time);
    uint32_t unused;
    if (!xin__system_property_wait(pi, data.last_read_serial, &unused, &ts))
      return false;
  }
}
...
const char *CachedProperty::Get(bool *changed) {
  ...
  if (prop_info_ == nullptr) {
    ...
    uint32_t property_area_serial = xin__system_property_area_serial();
  ...
  if (prop_info_ != nullptr) {
    ...
    uint32_t property_serial = xin__system_property_serial(prop_info_);
