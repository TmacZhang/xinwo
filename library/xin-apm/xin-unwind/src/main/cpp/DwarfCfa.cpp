...
bool DwarfCfa<AddressType>::GetLocationInfo(uint64_t pc, uint64_t start_offset, uint64_t end_offset,
  ...
  while (true) {
    ...
    case 2: {
      ...
      SignedType signed_offset = static_cast<SignedType>(offset) * fde_->cie->data_alignment_factor;
...
// Static data.
template <typename AddressType> bool DwarfCfa<AddressType>::cfa_nop(dwarf_loc_regs_t *) {
  return true;
}

template <typename AddressType> bool DwarfCfa<AddressType>::cfa_set_loc(dwarf_loc_regs_t *) {
...
template <typename AddressType> bool DwarfCfa<AddressType>::cfa_advance_loc(dwarf_loc_regs_t *) {
  cur_pc_ += operands_[0] * fde_->cie->code_alignment_factor;
  return true;
}

template <typename AddressType> bool DwarfCfa<AddressType>::cfa_offset(dwarf_loc_regs_t *loc_regs) {
