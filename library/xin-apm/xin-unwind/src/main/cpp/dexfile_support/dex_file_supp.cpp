...
bool TryLoadLibdexfileExternal([[maybe_unused]] std::string *err_msg) {
  ...
  *err_msg = "Dex file support not available.";
  ...
  if (!is_loaded) {
    ...
    void *handle = xin::linker::DlFcn::dlopen(so_name, RTLD_NOLOAD | RTLD_NOW | RTLD_NODELETE);
    if (handle == nullptr) {
      so_name = "libdexfile_external.so";
      handle = xin::linker::DlFcn::dlopen(so_name, RTLD_NOW | RTLD_GLOBAL | RTLD_NODELETE);
    }
    if (handle == nullptr) {
      *err_msg = dlerror();
      return false;
...
#define RESOLVE_DLFUNC_PTR(CLASS, DLFUNC)                                                          \
  decltype(DLFUNC) *DLFUNC##_ptr =                                                                 \
      reinterpret_cast<decltype(DLFUNC) *>(xin::linker::DlFcn::dlsym(handle, #DLFUNC));           \
  if ((DLFUNC) == nullptr) {                                                                       \
    *err_msg = dlerror();                                                                          \
    return false;                                                                                  \
  }
...
#undef SET_DLFUNC_PTR
    ...
    is_loaded = true;
    xin::linker::DlFcn::dlclose(handle);
  }
