...
Symbols::Symbols(uint64_t offset, uint64_t size, uint64_t entry_size, uint64_t str_offset,
                 uint64_t str_size)
    : offset_(offset), count_(entry_size != 0 ? size / entry_size : 0), entry_size_(entry_size),
      str_offset_(str_offset), str_end_(str_offset_ + str_size) {}

template <typename SymType> static bool IsFunc(const SymType *entry) {
...
inline __attribute__((__always_inline__)) const Symbols::Info *
Symbols::ReadFuncInfo(uint32_t symbol_index, Memory *elf_memory) {
...
template <typename SymType> void Symbols::BuildRemapTable(Memory *elf_memory) {
