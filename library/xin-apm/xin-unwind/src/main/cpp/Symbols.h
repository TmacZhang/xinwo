...
private:
  template <typename SymType> const Info *ReadFuncInfo(uint32_t symbol_index, Memory *elf_memory);

  template <typename SymType, bool RemapIndices>
  const Info *BinarySearch(uint64_t addr, Memory *elf_memory);

  template <typename SymType> void BuildRemapTable(Memory *elf_memory);
