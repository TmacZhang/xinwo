...
XIN_EXPORT void Unwinder::Unwind(const std::vector<std::string> *initial_map_names_to_skip,
...
XIN_EXPORT std::string Unwinder::FormatFrame(const FrameData &frame) const {
  std::string data;
  if (regs_->Is32Bit()) {
...
XIN_EXPORT std::string Unwinder::FormatFrame(size_t frame_num) const {
...
XIN_EXPORT bool UnwinderFromPid::Init(ArchEnum arch) {
  if (pid_ == getpid()) {
...
XIN_EXPORT FrameData Unwinder::BuildFrameFromPcOnly(uint64_t pc) {
