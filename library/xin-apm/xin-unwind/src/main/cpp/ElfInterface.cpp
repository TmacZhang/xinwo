...
template <typename AddressType> void ElfInterface::InitHeadersWithTemplate() {
...
template <typename EhdrType, typename PhdrType> int64_t ElfInterface::GetLoadBias(Memory *memory) {
  EhdrType ehdr;
...
void ElfInterface::ReadProgramHeaders(const EhdrType &ehdr, int64_t *load_bias) {
  ...
  for (size_t i = 0; i < ehdr.e_phnum; i++, offset += ehdr.e_phentsize) {
    ...
    case PT_LOAD: {
      ...
      pt_loads_[phdr.p_offset] =
          LoadInfo{phdr.p_offset, phdr.p_vaddr, static_cast<size_t>(phdr.p_memsz)};
...
template <typename NhdrType> std::string ElfInterface::ReadBuildID() {
...
template <typename DynType> std::string ElfInterface::GetSonameWithTemplate() {
...
template std::string
ElfInterface::ReadBuildIDFromMemory<Elf32_Ehdr, Elf32_Shdr, Elf32_Nhdr>(Memory *);
template std::string
ElfInterface::ReadBuildIDFromMemory<Elf64_Ehdr, Elf64_Shdr, Elf64_Nhdr>(Memory *);
