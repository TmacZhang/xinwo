...
template <typename AddressType> bool DwarfSectionImpl<AddressType>::FillInCieHeader(DwarfCie *cie) {
...
template <typename AddressType> bool DwarfSectionImpl<AddressType>::FillInCie(DwarfCie *cie) {
...
template <typename AddressType> bool DwarfSectionImpl<AddressType>::FillInFdeHeader(DwarfFde *fde) {
  uint32_t length32;
  if (!memory_.ReadBytes(&length32, sizeof(length32))) {
    last_error_.code = DWARF_ERROR_MEMORY_INVALID;
    last_error_.address = memory_.cur_offset();
...
template <typename AddressType> bool DwarfSectionImpl<AddressType>::FillInFde(DwarfFde *fde) {
...
template <typename AddressType> struct EvalInfo {
...
template <typename AddressType> void DwarfSectionImpl<AddressType>::InsertFde(const DwarfFde *fde) {
