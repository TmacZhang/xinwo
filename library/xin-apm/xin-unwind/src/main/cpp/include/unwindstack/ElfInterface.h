...
public:
  ...
  template <typename EhdrType, typename PhdrType> static int64_t GetLoadBias(Memory *memory);

  template <typename EhdrType, typename ShdrType, typename NhdrType>
  static std::string ReadBuildIDFromMemory(Memory *memory);

protected:
  template <typename AddressType> void InitHeadersWithTemplate();
  ...
  template <typename EhdrType, typename ShdrType> void ReadSectionHeaders(const EhdrType &ehdr);

  template <typename DynType> std::string GetSonameWithTemplate();
  ...
  template <typename EhdrType> static void GetMaxSizeWithTemplate(Memory *memory, uint64_t *size);

  template <typename NhdrType> std::string ReadBuildID();
