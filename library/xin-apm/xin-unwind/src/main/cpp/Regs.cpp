...
XIN_EXPORT Regs *Regs::RemoteGet(pid_t pid) {
...
XIN_EXPORT Regs *Regs::CreateFromUcontext(ArchEnum arch, void *ucontext) {
  switch (arch) {
  case ARCH_X86:
...
XIN_EXPORT ArchEnum Regs::CurrentArch() {
#if defined(__arm__)
  return ARCH_ARM;
...
XIN_EXPORT Regs *Regs::CreateFromLocal() {
  Regs *regs;
#if defined(__arm__)
  regs = new RegsArm();
#elif defined(__aarch64__)
