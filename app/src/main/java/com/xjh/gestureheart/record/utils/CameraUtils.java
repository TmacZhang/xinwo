...
public class CameraUtils {
    ...
    public static int[] choosePreviewSize(Camera.Parameters parms, int width, int height) {
        ...
        if (ppsfv != null) {
            Log.e(TAG, "CameraInfo Camera preferred preview size for video is " + ppsfv.width + "x" + ppsfv.height);
        }

        if (DEBUG) {
            for (Camera.Size size : parms.getSupportedPreviewSizes()) {
                Log.e(TAG, "CameraInfo supported: " + size.width + "x" + size.height);
            }
        }
        ...
        Log.e(TAG, "CameraInfo Unable to set preview size to " + width + "x" + height);
        if (ppsfv != null) {
