...
package com.xjh.gestureheart.record.gles;
...
public class FullFrameRect {
    ...
    //TODO UPDTA 2019-12-8: 解决相机预览画面被压缩或拉伸的问题
    public void drawFrame(int textureId, float[] mvpMatrix, float[] texMatrix, FloatBuffer vertexArray){
        if (mProgram == null) {
            return;
        }
        // Use the identity matrix for MVP so our 2x2 FULL_RECTANGLE covers the viewport.
        mProgram.draw(mvpMatrix, vertexArray, 0,
                mRectDrawable.getVertexCount(), mRectDrawable.getCoordsPerVertex(),
                mRectDrawable.getVertexStride(),
                texMatrix, mRectDrawable.getTexCoordArray(), textureId,
                mRectDrawable.getTexCoordStride());
